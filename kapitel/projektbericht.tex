...
In Schaubild \ref{fig:cvss-selection-process-selection} ist dieser gesamte Prozess einmal abgebildet.

\begin{enumerate}
\item Detaillierte Erklärungen sind in Kapitel \ref{subsubsec:projektbericht-loesungsweg-cvss-selection-rules-1} zu finden.
...
\item Detaillierte Erklärungen sind in Kapitel \ref{subsubsec:projektbericht-loesungsweg-cvss-selection-rules-2} zu finden.
...
Ein Beispiel für diese Schritte kann in Schaubild \ref{fig:cvss-selection-process-selection} gefunden werden.
Ein Auszu der Klasse mit ihren Attributen, die für die Selektion aus Schritt 1 zuständig ist, kann in Listing \ref{lst:cvss-selector-class-attributes} gesehen werden.
...
\paragraph{Regeln zur Vektor-Selektion und Kombination}

\paragraph{Statistics Evaluators (Statistik-Auswerter)}

\paragraph{Vector Evaluators}

\subsubsection{Schritt 2: Versions-Selektionsregeln} \label{subsubsec:projektbericht-loesungsweg-cvss-selection-rules-2}

Dieser Schritt ist wesentlich einfacher gestaltet, da er nur noch angeben muss, von welcher Vektor-Version der effektive Vektor sein soll.
Die Optionen hier sind die Folgenden TODO.
...
\caption{Selektionsvorgang für CVSS-Vektoren}
