...
In Listing \ref{lst:cvss-selector-version-selector-example-initial} ist der Selektor aufgeführt, der im Folgenden als \qt{initial} bezeichnet wird.

\begin{lstlisting}[language={}, label={lst:cvss-selector-version-selector-example-initial}, caption={Initialer CVSS-Selektor}]
...
In Listing \ref{lst:cvss-selector-version-selector-example-context} ist der Selektor aufgeführt, der im Folgenden als \qt{context} bezeichnet wird.
Er enthält die Basisregel des \qt{initial}-Selektors, und wird durch Regeln bezüglich der Assessment-Vektoren ergänzt, um eine kontextualisierte Sicht zu ermög.

\begin{lstlisting}[language={}, label={lst:cvss-selector-version-selector-example-context}, caption={Initialer CVSS-Selektor}]
...
\begin{figure}[htbp] % here, top, bottom, separate page
    \centering
    \includegraphics[width=1\textwidth, keepaspectratio]{res/grafiken/cvss-selection-process-selection-4}
